...
\end{equation}
%
where $\Delta U_r$ is a pre-determined normalized velocity deficit at the rotor hub. To avoid over-predicting the velocity deficits, and potentially causing an unrealistic flow reversal,  we used a value of $\Delta U_r=\Delta U_{d_m}$. 
...
The algorithm for calculating local turbulence intensity using the hard maximum function is given in \Cref{alg:ti}. In our code implementation, we combined the nested loops used for the velocity and turbulence intensity calculations. \todo{double check the algorithm and update if needed}
%
