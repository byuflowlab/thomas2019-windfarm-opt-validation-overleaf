...
\begin{abstract}
Recent work has demonstrated that performing wind farm layout optimization with gradient-based optimization methods is possible, but the improvements shown with gradient-based optimization methods have yet to be validated. Validation of the results with simulations or wind tunnel tests is important because the models used during optimization are simple and model characteristics may influence the resulting layouts. In this paper we begin by describing the models used, including changes specific for use with gradient-based optimization. We then compare our version of the models with the original versions and large eddy simulation (LES) data from previous publications. Using the models described, we performed gradient-based wind farm layout optimization, using exact gradients, and compare the results to LES simulations. The models predicted an improvement in annual energy production (AEP) of $7.4\%$, while the LES reported an AEP improvement of $9.9\%$. We conclude that the improvements found by optimizing with the simple wake model are not just an artifact of the models, but are real improvements.
\end{abstract}
...
In order to solve wind farm layout optimization problems (WFLOPs) using a reasonable amount of computational resources and time, WFLOPs are usually solved using simple engineering models. However, the models commonly used for the WFLOPs make simplifying assumptions that could potentially influence the final layout. Because of the simplified nature of the models, there is some uncertainty about whether the improvements reported are real improvements or just artifacts of unrealistic characteristics of the models themselves. 

Despite using simple models during optimization, the WFLOP is a difficult optimizaiton problem, classified as NPO-complete \cite{acero2014}. Because of the complexity of WFLOPs, the design space is often devided into a grid space of possible locations \cite{turner2014-grid-milp-wflo} or a regular structure is imposed by only allowing array-like layouts and optimizing with parameters such as turbine row spacing and angle \cite{perez-moreno2018}.  However, there is interest in unstructured, or irregular, wind farm layout optimization. Irregular WFLO does not impose any artificial limitations on the layout, potentially allowing more optimal layouts \cite{chowdhury2012}. However, due to the reduced restrictions, unrestricted optimization could also be more susceptible to unrealistic characteristics of models used during optimization. 

Because it is not practical to build and rebuild a windfarm to confirm the legitimacy of the improvements due to optimization with simple models, we are left with wind tunnel experiments and higher-order models such as large eddy simulation (LES). While many studies have investigated the accuracy of many simple wake models, few have investigated the validity of the optimization results obtained with the models. Some work has been performed on regular, or restricted, WFLOP solutions CITE. The only validation activies we are aware of for irregular or unstructured WFLOPs were performed using wind tunnel experiments and involved a small wind farm (nine turbines) and unidirectional flow \cite{chowdhury2012}. The complexity of the WFLOP comes from large numbers of wind turbines and probabilistic wind in many directions, so it is important to invetigate the results of unrestricted WFLOPs involving larger wind farms and many wind directions. 

Large numbers of wind turbines cause a corresponding increase in the number of design variables and constraints used by the optimization algorithm, which can reduce the effectiveness and efficiency of the algorithm. While the WFLOP is typically solved using gradient-free optimization methods \cite{acero2014}, grandient-free optimization methods exhibit reduced performance when applied to high dimensional problems \cite{rios2013-grad-free-comparison}. In \cite{rios2013-grad-free-comparison}, most gradient-free solvers were unable to solve more than $15\%$ of the test problems studied with 10-30 design variables, and even the best methods tested could solve only $28\%$ of the test problems with 31-300 design variables. New wind farms can include well over 100 turbines, meaning that unrestricted WFLOPs can have over 200 variables and thousands of constraints. In short, the variables and constraints of the unrestricted WFLOPs of interest exceed the abilities of most gradient-free optimization methods.
...
The algorithm for calculating local turbulence intensity using the hard maximum function, TI method (3), is given in \Cref{alg:ti}. In our code implementation, we combined the nested loops used for the velocity and turbulence intensity calculations. \todo{double check the algorithm and update if needed}
%
