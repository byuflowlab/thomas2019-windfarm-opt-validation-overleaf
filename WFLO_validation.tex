...
Because it is not practical to build and rebuild a windfarm to confirm the legitimacy of the improvements due to optimization with simple models, we are left with wind tunnel experiments and higher-order models such as large eddy simulation (LES). While many studies have compared many simple wake models to LES and wind tunnel results (such as \cite{crespo1999,barthelmie2005,barthelmie2009,gaumond2012,beaucage2012, annoni2014,moriarty2014,annoni2016,andersen2014,gocmen2016,machefaux2015}), few have investigated the validity of the optimization results obtained with the models. One study (\cite{bokharaie2016}) involved LES in hybrid, or multi-fidelity, optimization with the Jensen ``tophat" wake model. LES was used for validation and for tuning during optimization using 30 turbines and up to 8 wind directions. This study showed similar improvements according to both the LES and the Jensen model, but avoided some of the inherent inaccuracy of the simple model by adjusting it iteratively based on LES results. While this approach worked well, it was still much more costly computationally than optimization using only simple models. The only other validation activity we are aware of for irregular or unstructured WFLOPs were performed using wind tunnel experiments and involved a small wind farm (nine turbines) and unidirectional flow \cite{chowdhury2012}. The complexity of the WFLOP comes from large numbers of wind turbines and probabilistic wind in many directions, so it is important to investigate the results of unrestricted WFLOPs involving larger wind farms and many wind directions. 
