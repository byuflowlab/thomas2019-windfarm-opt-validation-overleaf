...
\end{figure}
When comparing our code to LES and other data, we used 100 sampling points accross the rotor-swept area arranged in the sunflower seed pattern as shown in \Cref{fig:sampling_locs}(a) to minimize the model error. Using more than 100 points did not significantly alter the results. For efficiency during optimization, we approximated the effective velocity of each wind turbine by using a single sampling point located at the rotor hub as shown in \Cref{fig:sampling_locs}(b).
...
%\begin{tabularx}{\textwidth}{ |X|X|X|X| }\label{tab:np-error-comparison}
%  \hline
